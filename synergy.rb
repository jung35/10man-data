...
(0..count-1).each do |row|
  player_synergy[player_list[row][:id]] = {}
  (0..count-1).each do |col|
    next if row == col
    player_synergy[player_list[row][:id]][player_list[col][:id]] = { win: 0, lose: 0, tie: 0 }
  end
end
...
matches.each do |list|
  (1..2).each do |i|
    ...
    list["team#{i}"].each do |row_player|
      list["team#{i}"].each do |col_player|
        next if row_player["id"] == col_player["id"]

        temp_syn = player_synergy[row_player["id"]][col_player["id"]]

        temp_syn[:win] += (won ? 1 : 0)
        temp_syn[:lose] += (lost ? 1 : 0)
        temp_syn[:tie] += (tied ? 1 : 0)

        player_synergy[row_player["id"]][col_player["id"]] = temp_syn
      end
    end
  end
end
...
data = {
  ...
  :match_data => player_synergy
}

puts 'Saving data'
File.open('player_synergy.json', 'w') do |f|
